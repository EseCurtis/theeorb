...
export function CvVaultScreen(): React.JSX.Element {
  ...
  return <ScreenLayout description="Your CV is private and attached only after your final send review." title="CV vault"><Panel label="Upload a CV" tone="cyan"><Text className="text-sm leading-6 text-[var(--muted)]">Use a PDF or DOCX, up to 10 MB.</Text><label className="flex min-h-12 cursor-pointer items-center justify-center rounded-xl bg-[#0a4147] px-4 text-sm text-[#d4fffb]"><Text>{isUploadingDocument ? 'Uploading…' : 'Choose CV file'}</Text><input accept=".pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" className="sr-only" disabled={isUploadingDocument} onChange={(event) => void handleFile(event)} type="file" /></label></Panel>{message ? <Text className="text-sm text-[#b7fffa]">{message}</Text> : null}<View className="gap-3">{careerDocuments.map((document) => <Panel key={document.id} label="Private document" tone="violet"><Text className="text-base text-[var(--foreground)]">{document.originalFilename}</Text><Text className="text-sm text-[var(--muted)]">{Math.round(document.byteSize / 1024)} KB · {document.mimeType.includes('pdf') ? 'PDF' : 'DOCX'}</Text></Panel>)}</View></ScreenLayout>
}
